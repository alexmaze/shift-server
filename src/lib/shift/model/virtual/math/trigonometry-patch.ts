...
export class TrigonometryPatch extends NodeModel {
  ...
  handle(nodes) {
    ...
    switch (this.operation) {
      case "sin":
        this.outputs[0].value = Math.sin(originValue)
        break
      case "cos":
        this.outputs[0].value = Math.cos(originValue)
        break
      case "tan":
        this.outputs[0].value = Math.tan(originValue)
        break
      default:
        throw "ERROR"
    }
